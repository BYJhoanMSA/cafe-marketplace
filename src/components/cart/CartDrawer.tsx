...
import { checkProductsExist } from '@/server/actions/cart.actions'
...
export function CartDrawer() {
  ...
  useEffect(() => {
    ...
    checkProductsExist(productIds).then(setUnavailableIds)
  }, [isCartOpen, items])

  const availableItems = items.filter(i => !unavailableIds.has(i.productId))
  const currency = items[0]?.currency || 'USD'
  const availableSubtotal = availableItems.reduce(
    (acc, i) => acc + i.priceInCents * i.quantity, 0
  )
  const whatsappUrl = buildWhatsAppOrderUrl(availableItems, availableSubtotal, currency)
        ...
        {items.length === 0 ? (
          ...
          <>
            <div className={styles.itemList}>
              {items.map((item) => {
                const unavailable = unavailableIds.has(item.productId)
                return (
                  <div key={item.id} className={`${styles.item}${unavailable ? ` ${styles.unavailable}` : ''}`}>
                    {item.imageUrl ? (
                      <Image
                        src={item.imageUrl}
                        alt={item.title}
              ...
              <div className={styles.footer}>
                <div className={styles.summaryRow}>
                  ...
                  <span className={styles.summaryTotal}>
                    {formatPrice(availableSubtotal, currency)}
                  </span>
                </div>
